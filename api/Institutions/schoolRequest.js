'use strict';
console.log('Loading schoolRequest.handler');
/*
	Lambda Function for schoolRequest.handler in crosscampus by xcampus API
...
exports.handler = (event, context) => {
    ...
    if(event.pathParameters) {
        request = parseRequest(event.pathParameters, request);
    }
    ...
    switch(request.httpMethod) {
        ...
        case 'DELETE':
            deleteHandler(request, request.username, done);
            break;
    ...
    function getHandler(entry,queryParams, done) {
        ...
        let query = "SELECT entry.*, count(rating.userID) AS rating FROM entry "
                  + "LEFT JOIN rating ON entry.ID=rating.entryID WHERE "
        if(entry.id){
            ...
            console.log(
                connection.query(
                    query, (error, entries) => {        
                        ...
                        if(!entries) reject(new Error("Error : No entries found!"));
