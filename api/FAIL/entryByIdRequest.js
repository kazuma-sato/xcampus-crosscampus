...
exports.handler = (event, context) => {
    ...
    function getHandler(entry,queryParams, done) {
        ...
        if(entry.id){
            query = mysql.format(query + "id=? AND ", entry.id);
        } else {
        ...
        query += "flaggedBy IS NULL ";
        query += queryParams.recent ? "ORDER BY dateCreated ASC " : "";
        if(queryParams.limit) {
            query = mysql.format(
                query + "LIMIT " + (queryParams.limit.length == 1)?"?":"?,? "
                , queryParams.limit);
        }
